...
from .thread_agents import ThreadFacadeAgent, _AGENT_REGISTRY
...
class ThreadAgentRunner:
    def __init__(self, agent: ThreadFacadeAgent, debug: str | bool = False) -> None:
        self.facade = agent
        if debug:
    ...
    def report_usages(completions: list[FinishCompletion]):
        aggregator = Aggregator()
        for row in ThreadAgentRunner.print_stats_report(completions, aggregator):
            print(row)
    ...
    def rich_loop(self):    
        ...
        try:
            ...
            for row in self.print_stats_report(saved_completions, Aggregator()):
                console.out(row)
            readline.write_history_file(os.path.expanduser("~/.agentic_history"))
        except EOFError:
            print("\nExiting REPL.")
            return
        except KeyboardInterrupt:
