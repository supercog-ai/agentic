...
# Threading thread-local storage to maintain depth
class ThreadLocal(threading.local):
    def __init__(self):
        self.depth = 0

depthLocal = ThreadLocal()

# Class to handle thread results, similar to Ray's ObjectRef
class ThreadResult:
    def __init__(self):
        self._result = None
...
# Global registry for agents (to replace Ray's actor registry)
_AGENT_REGISTRY = []

# A simple app for the FastAPI part
app = FastAPI()
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class ActorBaseAgent:
    ...
    # Thread-safe method calling to replace remote
    def remote(self, method_name, *args, **kwargs):
        ...
        def thread_func():
            try:
                ...
                result.set_result(method_result)
    ...
    def call_child(
        ...
        request_start = child_ref.start_request(
            message,
            debug=self.debug
        )
        
        for event in child_ref.get_events(request_start.request_id):
            yield event
...
def handoff(agent, **kwargs):
    """Small utility to create a HandoffAgentWrapper around an agent. This is useful for
    creating a child agent that will be handed off to (a one-way delegation)."""
    child_params = {}
...
class DynamicFastAPIHandler:
    def __init__(self, actor_ref, agent_facade):
    ...
    def bind(self):
        # This is a replacement for Ray's bind functionality
        # In a thread-based implementation, we just return self
        return self
    ...
    async def stream_request(self, prompt: ProcessRequest) -> EventSourceResponse:
        def render_events():
            request_id = self.agent_facade.start_request(prompt.prompt).request_id
            for event in self.agent_facade.get_events(request_id):
    ...
    async def get_runs(self) -> list[dict]:
        """Returns the list of runs for the agent"""
        return [run.model_dump() for run in self.agent_facade.get_runs()]

    @app.get('/runs/{run_id}/logs')
    async def get_run_logs(self, run_id=str) -> list[dict]:
        """Returns the logs for a specific run"""
        return [log.model_dump() for log in self.agent_facade.get_run_logs(run_id)]
    ...
    def next_turn(
        ...
        if not continue_result:
            for event in self._agent.handlePromptOrResume(
                Prompt(
                    self.name,
                    request,
                    depth=0,
...
class ThreadFacadeAgent:
    """The facade agent is the object directly instantiated in code. It holds a reference to the thread-based
    agent that does the actual work, and manages event queues, etc."""
    ...
    def __init__(
        ...
        self.name = name
        self.welcome = welcome or f"Hi, I'm {name}. How can I help you?"
        ...
        if mock_settings and self.model and "mock" in self.model:
            ...
            if hasattr(self, '_agent'):
                try:
                    ...
                    print(f"Warning: Failed to set mock params on agent: {e}")
    ...
    def _init_base_actor(self, instructions: str):
        ...
        self._agent = ActorBaseAgent(name=self.name)
    ...
    def _create_fastapi_endpoint(self, port: int = 8086):
        ...
        # Create a FastAPI app and uvicorn server
        import uvicorn
        from threading import Thread

        dep = DynamicFastAPIHandler(self._agent, self)
        dep.name = f"{self.safe_name}_api"
        deployment = dep

        api_endpoint = f"http://0.0.0.0:{port}/{self.safe_name}"
        self._agent.api_endpoint = api_endpoint

        if base_serve_app is None:
            base_serve_app = BaseServeDeployment()
    ...
    def _get_funcs(self, thefuncs: list):
        ...
        for func in thefuncs:
            if callable(func):
                ...
                useable.append(func)
            elif isinstance(func, ThreadFacadeAgent):
                # add a child agent as a tool
                useable.append(
                    AddChild(
                        func.name,
                        func,  # Pass the agent itself, not ._agent
                    )
                )
                ...
                useable.append(
                    AddChild(
                        func.get_agent().name,
                        func.get_agent(),  # Pass the agent itself, not ._agent
                        handoff=True,
                    )
                )
            else:
    ...
    def prompt_variables(self) -> dict:
        ...
        if self.template_path and os.path.exists(self.template_path):
            try:
                ...
                print(f"Warning: Error loading template file {self.template_path}: {e}")

        return vars_dict

    @property
    def safe_name(self) -> str:
        """Return a URL/file safe version of the agent name"""
    ...
    def next_turn(
        ...
        if not continue_result:
            ...
            for event in self._agent.handlePromptOrResume(prompt):
                if self.cancelled:
            ...
            for event in self._agent.handlePromptOrResume(ResumeWithInput(self.name, continue_result)):
                if self.cancelled:
                    raise TurnCancelledError()
                if isinstance(event, TurnEnd):
                    if isinstance(event.result, str) and self.result_model:
    ...
    def set_debug_level(self, level: DebugLevel):
        ...
        self._agent.set_debug_level(self.debug)
